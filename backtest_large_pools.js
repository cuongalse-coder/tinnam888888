/**
 * V750A BACKTEST FOR LARGE POOLS (30, 33, 35)
 * Testing if we can hit 50% 6/6 with larger pool sizes.
 */
...
function ensV750A(data, mx) {
    ...
    // Sector diversity: ensure at least top2 from each decade in pool-45
    const ranked = Object.entries(v).sort((a,b)=>b[1]-a[1]).map(x=>parseInt(x[0]));
    const topN = ranked.slice(0, 35); // Max pool size we test
    const sectors = {};
    for (const num of topN) sectors[Math.floor((num-1)/10)] = (sectors[Math.floor((num-1)/10)]||0)+1;
    ...
    for (let s = 0; s < 5; s++) {
        if (!sectors[s] || sectors[s] < 2) {
            ...
            if (sectorNums.length > 0) {
                result.splice(result.length - 2, 1, sectorNums[0]); // Replace weakest in pool
            }
        }
    }
    // Return all 45 ranked
    for(const n of ranked) if(!result.includes(n)) result.push(n);
    return result;
}

function runTest(allData, mx, fn) {
    const c={20:{},25:{},30:{},33:{},35:{}}; for(const p of[20,25,30,33,35])for(let k=0;k<=6;k++)c[p][k]=0; let n=0;
    for(let ci=60;ci<allData.length;ci++){const hist=allData.slice(0,ci),actual=new Set(allData[ci]),ranked=fn(hist,mx);for(const p of[20,25,30,33,35]){const ts=new Set(ranked.slice(0,p));let h=0;for(const x of actual)if(ts.has(x))h++;c[p][h]++;}n++;}
    return{c,n};
}

async function main() {
    console.log('🧬 TESTING LARGE POOLS FOR 50% 6/6 HIT RATE');
    console.log('='.repeat(65));
    const allData = await fetchData();
    console.log(`✅ ${allData.length} draws\n`);
    
    const r = runTest(allData, 45, ensV750A);
    
    console.log('Pool Size'.padEnd(15) + '6/6 Hits'.padEnd(15) + 'Win Rate (%)');
    console.log('-'.repeat(45));
    
    for (const p of [20, 25, 30, 33, 35]) {
        const hits = r.c[p][6];
        const rate = (hits / r.n * 100).toFixed(1);
        console.log(`Pool-${p}`.padEnd(15) + `${hits}/${r.n}`.padEnd(15) + `${rate}%`);
    }
}

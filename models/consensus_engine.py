...
class ConsensusEngine:
    ...
    def predict(self, data, dates=None, n_portfolio=50):
        ...
        # ============================================================
        weights = self._walk_forward_calibrate(data, dates)
        ...
        # ============================================================
        # PHASE 8: Walk-forward backtest summary (runs BEFORE confidence)
        # ============================================================
        bt_summary = self._quick_backtest(data, dates, constraints)

        # ============================================================
        # PHASE 9: Confidence Calculation (based on honest backtest)
        # ============================================================
        confidence = self._calc_confidence(bt_summary)
    ...
    # ================================================================
    # SIGNAL COMPUTATION HELPER (extracted to avoid data leakage)
    # ================================================================
    def _compute_all_signals(self, data, dates=None):
        """Compute all signals from the given data subset.

        This is extracted so that walk-forward calibration and backtest
        can recompute signals on training-only data, avoiding leakage.
        """
        signals = {}
    ...
    # ================================================================
    def _walk_forward_calibrate(self, data, dates=None):
        """Proper walk-forward: re-compute signals on training data only.

        Splits data into train/test, computes signals ONLY on train,
        then evaluates against test draws. No data leakage.
        """
        n = len(data)
        test_count = min(50, n - 70)
        # Need a default signal set to return keys
        all_signal_names = self._compute_all_signals(data[:70], dates[:70] if dates and len(dates) >= 70 else None).keys() if n >= 70 else []
        if test_count < 10:
            return {name: 1.0 for name in all_signal_names}

        # Compute signals on TRAINING data only (exclude test draws)
        train_data = data[:-test_count]
        train_dates = dates[:-test_count] if dates else None
    ...
    # ================================================================
    def _calc_confidence(self, backtest_summary):
        """Calculate confidence 0-100 based on honest backtest results.

        Uses walk-forward backtest improvement over random baseline
        instead of self-referencing signal agreement (which was a
        data leakage bug — signals already "saw" the prediction).
        """
        if not backtest_summary or backtest_summary.get('tests', 0) < 5:
            return 30.0
        improvement = backtest_summary.get('improvement', 0)
        # Scale: 0% improvement = 30, 50% = 55, 100% = 80, 150%+ = 90+
        confidence = 30 + min(improvement * 0.4, 65)
        return max(min(confidence, 95), 15)

    # ================================================================
    # QUICK BACKTEST
    # ================================================================
    def _quick_backtest(self, data, dates, constraints):
        """Quick walk-forward backtest of the consensus approach.

        Recomputes signals on TRAINING data only, then evaluates
        predictions against held-out test draws. No data leakage.
        """
        n = len(data)
        ...
        # Compute signals and weights on training data only
        train_data = data[:-test_count]
        train_dates = dates[:-test_count] if dates else None
        train_signals = self._compute_all_signals(train_data, train_dates)

        # Calibrate weights on training data (use equal weights to avoid
        # circular dependency with _walk_forward_calibrate)
        train_weights = {name: 1.0 for name in train_signals}

        matches = []
        for idx in range(n - test_count - 1, n - 1):
            actual = set(data[idx + 1][:6])

            # Build consensus from train-only signals
            consensus = {num: 0.0 for num in range(1, self.max_number + 1)}
            for sig_name, sig_scores in train_signals.items():
                if not sig_scores:
                    continue
                w = train_weights.get(sig_name, 1.0)
                vals = list(sig_scores.values())
                max_v = max(abs(v) for v in vals) if vals else 1
                if max_v < 0.001:
                    max_v = 1
